        ...
        <div class="flex items-start gap-3 px-4 py-3.5">
            ...
            <div class="flex-1 min-w-0">
                ...
                @foreach ($errors->all() as $error)
                    <p class="text-xs {{ $isDark ? 'text-red-300/80' : 'text-red-600/80 dark:text-red-400/80' }}">{{ $error }}</p>
                @endforeach
                ...
                @if (!is_null($attemptsRemaining))
                    <div class="flex items-center gap-1.5 mt-2 pt-2"
                         style="border-top: 1px solid rgba(239,68,68,0.22);">
                        <div class="flex gap-0.5">
                            @for ($i = 0; $i < 3; $i++)
                                <div class="w-4 h-1.5 rounded-full {{ $i < (3 - $attemptsRemaining) ? 'bg-red-400' : ($isDark ? 'bg-white/20' : 'bg-red-200') }}"></div>
                            @endfor
                        </div>
...
<form method="POST" action="{{ route('login.store') }}" autocomplete="off">
    ...
    <div class="mb-4">
        ...
        <div class="relative">
            <div class="absolute inset-y-0 left-3.5 flex items-center pointer-events-none">
                <svg class="w-4 h-4 {{ $isDark ? 'text-white/40' : 'text-gray-400 dark:text-gray-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <input type="text"
                   name="username"
                   id="{{ $isDark ? 'username_m' : 'username_d' }}"
                   value="{{ old('username') }}"
    ...
    <div class="mb-6">
        ...
        <div class="relative">
            ...
            <input :type="showPassword ? 'text' : 'password'"
                   name="password"
                   id="{{ $isDark ? 'password_m' : 'password_d' }}"
                   placeholder="{{ __('Masukkan password') }}"
                   autocomplete="off"
                   class="{{ $inputBase }} {{ $hasErrors ? $inputError : $inputNormal }} pl-10 pr-11" />
            {{-- Show/hide toggle --}}
            <button type="button"
    ...
    <div class="mt-5 flex items-center justify-center gap-1.5">
        <svg class="w-3.5 h-3.5 {{ $isDark ? 'text-white/30' : 'text-gray-400 dark:text-gray-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
